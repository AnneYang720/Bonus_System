<template>
  <div>
    ...
    <div>
        ...
        <el-button style="margin-left:2%" type="primary" @click="fetchManAllDetail()">选择</el-button>
...
export default {
    ...
    methods: {
        ...
        fetchManAllDetail(){
            ...
            generalApi.getManAllDetail(this.curChosenPlan, this.currentPage,this.pageSize).then(response =>{
                this.total = response.total
                this.manDetailList = response.detail;
        ...
        handleCheck(row,project,projectname){
            ...
            generalApi.getManHistory(formData).then(response =>{
        ...
        exportExcel(){
          ...
          if(fix){ //判断要导出的节点中是否有fixed的表格，如果有，转换excel时先将该dom移除，然后append回去
            ...
            for(var i=2; i<this.manDetailList.length+2; i++){
              var key = "C"+i
          ...
          try {
              FileSaver.saveAs(new Blob([wbout], { type: 'application/octet-stream' }), '管理项目奖金总览.xlsx')
          } catch (e) { 
