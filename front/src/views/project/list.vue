    ...
    <el-table
      ...
      <el-table-column width="90" v-for="(item,index) in projectList" :key="index" :prop="item.id" :label="item.name">
        <template slot-scope="scope">
          ...
          <span v-else style="color: Red" @click="handleCheck(scope.row, projectList[index].id, projectList[index].name, projectList[index].type)" >{{ scope.row[projectList[index].id] }}</span>
        </template>
      </el-table-column>    
...
export default {
    ...
    methods: {
        ...
        fetchAllDetail(){
            ...
            projectApi.getAllDetail(this.curChosenPlan).then(response =>{
        ...
        handleCheck(row,project,projectName, projectType){
            ...
            projectApi.getHistory(formData).then(response =>{
        ...
        exportExcel(){
            ...
            try {
                FileSaver.saveAs(new Blob([wbout], { type: 'application/octet-stream' }), '项目经理奖金总览.xlsx')
            } catch (e) { 
